...
function MysteryIndex({ dark }) {
 ...
 // const [color, setColor] = useState("Legendary");
  ...
  const [nftopencount, setNftopencount] = useState(-1);
  ...
  const handleClickOpen = () => {
    if(openBox === false && nftopencount + 1 < nftmintdata.length)
    {
      setOpenBox(true);
      setNftopencount(nftopencount + 1);  
    }
  }
  useEffect(() => {
      setNftopencount(-1);
        ...
        <S.Modal title={null} footer={null} closable={false} maskClosable={true} mask={true} visible={videoopen} mobile={width < height ? true : false}>
          <S.OpeningContainer>
            ...
            <div style={{width:'100%', height: width > height ?'100%' :  'calc(100vw * 4 / 3)'}}  className="player-wrapper">
            {
              useMemo(() =>
              (<>
                {nftmintdata.length > 0 && openBox && 
                    <ReactPlayer 
                      url= {width > height ? nftmintdata[nftopencount]['video']['desktop'] : nftmintdata[nftopencount]['video']['mobile']}
                      playing	={openBox && !allStop}
                      loop={false}
                      muted={muted}
              ...
              <div>
                ...
                <Row>
                  <S.OpenNumberContainer>
                    opening {nftopencount + 1}/{nftmintdata.length}...
                  </S.OpenNumberContainer>
                </Row>
              </div>            
              <div>
                <Row justify="center">
                  ...
                  <S.ImageBtn 
                    width={300}
                    height={120}
                    image_on={nftopencount >= 0 ? (nftopencount + 1=== nftmintdata.length ? openAlLViewBtnOn : openNextBtnOn) : openBtnOn} image={nftopencount >= 0 ?  (nftopencount + 1 === nftmintdata.length ? openAlLViewBtn : openNextBtn)  : openBtn}
